...
#
# Needs gcp-tts.sh on PATH (Gemini API key), jq and ffmpeg. One voice for the whole
...
VOICE="${VOICE:-Leda}"
MODEL="${MODEL:-gemini-2.5-pro-preview-tts}"
...
while IFS=$'\t' read -r id say; do
  ...
  for attempt in 1 2 3 4; do
    if gcp-tts.sh "$STYLE$say" "$wav" "$VOICE" "$MODEL" >/dev/null 2>"$TMP/err"; then ok=1; break; fi
